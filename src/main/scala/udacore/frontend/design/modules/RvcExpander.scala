...
import udacore.frontend.spec.modules.RvcExpanderSpecs._

@LocalSpec(contRvcExpander)
class RvcExpander(val params: FrontendParams) extends FrontendModule {
  val io = IO(new Bundle {
    @LocalSpec(intfCompressedIn)
    val in = Input(UInt(32.W))

    @LocalSpec(intfExpandedOut)
    val out = Output(UInt(32.W))
  })

  val usingCompressed = true

  @LocalSpec(funcRvcDecode)
  val rvcDecode = if (usingCompressed) {
    when(io.in(1, 0) =/= 3.U) {
...
class RVCDecoder(x: UInt, xLen: Int, useAddiForMv: Boolean = false) {
  ...
  def q1 = {
    ...
    def arith    = {
      ...
      def rtype = {
        ...
        val funct =
          VecInit(0.U, 4.U, 6.U, 7.U, 0.U, 0.U, 2.U, 3.U)(Cat(x(12), x(6, 5)))
  ...
  def decode = (VecInit(q0 ++ q1 ++ q2 ++ q3))(Cat(x(1, 0), x(15, 13)))
}
