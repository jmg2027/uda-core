...
import udacore.common.ControlSignal.MultiplierControl
...
/** MultiplierUnit (spec: MultiplierUnitSpecs; ADR-019, ADR-019C E-2).
  *
  * Wraps the external Multiplier (sliceWidth 32: the single-cycle production point) with one
  * in-flight operation and a one-entry result holder.
  *
...
class MultiplierUnit(val params: BackendParams) extends BackendModule {
  val io = IO(new Bundle {
    ...
    val multiplierReqIn = Flipped(Decoupled(new IssuedUop(params)))

    @LocalSpec(intfMultiplierResultOut)
    val multiplierResultOut = Decoupled(new FuResult(params))

    @LocalSpec(intfRecoveryEventIn)
    val recoveryEventIn = Input(new RecoveryEvent(params))
  })

  private val req = io.multiplierReqIn
  private val ev  = io.recoveryEventIn
  private val holder = new ResultHolder(new FuResult(params), (r: FuResult) => r.robTag, ev, io.multiplierResultOut)
  ...
  val integrateExternalMultiplier: Unit = {
    val core = Module(new Multiplier(MultiplierParams(dataWidth = xLen, sliceWidth = 32)))
    ...
    core.io.op    := Mux(busy, heldOp, req.bits.op)(MultiplierControl.getWidth - 1, 0).asTypeOf(MultiplierControl())
